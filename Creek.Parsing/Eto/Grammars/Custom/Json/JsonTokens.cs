...
namespace Creek.Parsing.Eto.Grammars.Custom.Json
{
    /// <summary>
    ...
    {
        private static JsonGrammar grammar;

        private static readonly JsonNull nullLiteral = new JsonNull();

        protected static JsonGrammar Grammar
        {
            get { return grammar ?? (grammar = new JsonGrammar()); }
        }
    ...
    public class JsonObject : JsonToken, IDictionary<string, JsonToken>
    {
        ...
        #region IDictionary<string,JsonToken> Members
        ...
        {
            Match match = GetProperty(Match, key);
            if (match != null)
            {
        ...
        {
            get { return GetToken(GetProperty(Match, index)); }
        ...
        public static object GetValue(Match match)
        {
            ...
            foreach (Match child in match.Matches)
            {
        ...
        private IEnumerable<KeyValuePair<string, JsonToken>> Enumerate()
        {
            for (int i = 0; i < Match.Matches.Count; i++)
            {
                Match match = Match.Matches[i];
                yield return
                    new KeyValuePair<string, JsonToken>(match.Matches[0].StringValue, GetToken(match.Matches[1]));
            }
        }
    ...
    public class JsonArray : JsonToken, IList<JsonToken>
    {
        ...
        #region IList<JsonToken> Members
        ...
        private class Enumerator : IEnumerator<JsonToken>
        {
