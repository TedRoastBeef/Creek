...
namespace Lib.Tools.TypeBuilder.Blocks
{
    using Creek.Tools;
    ...
    /// </summary>
	public class FluentBlockBuilder<TParentBuilder, TThis>:
		_IBlockBuilder,
		_ICompilable
	{
        ...
        internal void _CheckModifiable()
        {
            ...
            if (_readOnly)
                throw new ReadOnlyException("The actual block builder is read-only. This happens when you end the block or when the method that uses this block is compiled.");
        }
        ...
        public FluentUsingBuilder<TThis> Using<T1, T2>(Expression<Func<T1>> usingVariable, Expression<Func<T2>> usingValue)
        where
            T1: IDisposable
        where
            T2: T1
        {
            ...
            var result = new FluentUsingBuilder<TThis>(_this, _method, usingVariable.Body, usingValue.Body);
            _statements.Add(result);
            return result;
        }
        ...
        /// </summary>
		public TThis Assign<T1, T2>(Expression<Func<T1>> leftExpression, Expression<Func<T2>> rightExpression)
		where
			T2: T1
		{
        ...
        /// Starts a For loop.
        /// </summary>
		public FluentForBuilder<TThis> For<T>(Expression<Func<T>> initialization, Expression<Func<bool>> condition, Expression<Func<T>> increment)
		{
        ...
        /// Starts a For loop.
        /// </summary>
        public FluentForBuilder<TThis> For<T>(FluentExpression initialization, Expression<Func<bool>> condition, Expression<Func<T>> increment)
		{
        ...
        /// Starts a For loop.
        /// </summary>
        public FluentForBuilder<TThis> For<T>(Expression<Func<T>> initialization, Expression<Func<bool>> condition, FluentExpression increment)
		{
        ...
        /// Starts a For loop.
        /// </summary>
        public FluentForBuilder<TThis> For<T>(Expression<Func<T>> initialization, FluentExpression condition, FluentExpression increment)
		{
        ...
        /// Starts a For loop.
        /// </summary>
        public FluentForBuilder<TThis> For<T>(FluentExpression initialization, FluentExpression condition, Expression<Func<T>> increment)
		{
		...
		internal FluentForBuilder<TThis> _For(Expression initialization, Expression condition, Expression increment)
		{
            ...
            var result = new FluentForBuilder<TThis>(_this, _method, initialization, condition, increment);
			_statements.Add(result);
			return result;
		}
