...
namespace Lib.Tools.TypeBuilder
{
    ...
    internal sealed class _Visitor:
		ExpressionVisitor
	{
        ...
        protected override Expression VisitMethodCall(MethodCallExpression node)
        {
            ...
            if (method.Name == "Call")
            {
                ...
                if (declaringType == typeof(FluentVoidMethodBuilder) || declaringType.IsGenericType && declaringType.GetGenericTypeDefinition() == typeof(FluentMethodBuilder<>))
                {
                    ...
                    if (memberAccess != null)
                    {
                        ...
                        if (constantExpresion != null && member.MemberType == MemberTypes.Field)
                        {
                            ...
                            if (methodBuilder == null)
                                throw new InvalidOperationException("The expression is trying to call a null reference to a method.");

                            methodBuilder._CheckThread();

                            if (_method.TypeBuilder == null)
                                throw new NotSupportedException("At this moment only methods that are part of a type can call other methods.");

                            if (methodBuilder.TypeBuilder != _method.TypeBuilder)
                                throw new NotSupportedException("At this moment only methods of the same FluentTypeBuilder can be invoked.");

							if (methodBuilder._respectVisibility)
								throw new NotSupportedException("At this moment it is only possible to call another method that has the respectVisibility set to false.");

                            foreach(var parameter in methodBuilder._parameters.Values)
                            {
                                var parameterType = parameter.Type;
                                if (parameter.IsByRef || (parameterType.IsGenericType && parameterType.GetGenericTypeDefinition() == typeof(FluentMethodBuilder.Box<>)))
                                    throw new InvalidOperationException("InputAndOutput (ref) parameters are not supported when calling another method on the same type. Consider returning an struct with all the outputs you need instead.");
                            }
                            ...
                            Expression callExpression =
                                Expression.Call
                                (
                                    Expression.ArrayAccess(_method.TypeBuilder.DelegatesExpression, Expression.Constant(methodBuilder._delegateIndex)),
                                    typeof(Delegate).GetMethod("DynamicInvoke"),
                                    _RebuildArguments(methodBuilder, arguments[0])
                                );

                            if (methodBuilder.ReturnType != typeof(void)  && methodBuilder.ReturnType != typeof(object))
                                callExpression = Expression.Convert(callExpression, methodBuilder.ReturnType);
        ...
        private static readonly MethodInfo _addInstanceMethod = typeof(_Visitor).GetMethod("AddInstance");
			...
			if (member.MemberType == MemberTypes.Field)
			{
				...
				if (innerExpression == null || innerExpression.NodeType == ExpressionType.Constant)
				{
					...
					if (_method._parameters.TryGetValue(pair, out parameterExpression))
                    {
                        ...
                        if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(FluentMethodBuilder.Box<>))
                    ...
                    if (!field.IsStatic)
					    throw new InvalidOperationException("This expression is acessing a field (that is possible the result of an expression acessing a generator local variable) named \"" + field.Name + "\" which is not registered either as a field, parameter, local or external variable.");
				}
			}
