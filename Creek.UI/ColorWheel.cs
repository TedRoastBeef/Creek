...
namespace Creek.UI
{
    public class ColorWheel : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            ...
            var x = (int) Math.Round(d*Math.Cos(radAngle));
            var y = (int) -Math.Round(d*Math.Sin(radAngle));
            // Map center-relative coordinates to window coordinates
            x += wheelBitmap.Width/2;
            y += wheelBitmap.Width/2;
            Color c = ColorMath.ToGray(ColorMath.HslToRgb(new HslColor(hue, 255, 128))) > 128
                          ? Color.Black
                          : Color.White;
            ...
            if (secondaryHues != null)
            {
                foreach (byte sHue in secondaryHues)
                {
                    ...
                    x += wheelBitmap.Width/2;
                    y += wheelBitmap.Width/2;
                    c = ColorMath.ToGray(ColorMath.HslToRgb(new HslColor(sHue, 255, 128))) > 128
                            ? Color.Black
                            : Color.White;
            ...
            c = ColorMath.ToGray(ColorMath.HslToRgb(new HslColor(hue, saturation, lightness))) > 128
                    ? Color.Black
                    : Color.White;
            using (var p = new Pen(c))
            {
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ...
                else
                {
                    ...
                    var center = new Point(halfWheelWidth, halfWheelWidth);
            ...
            for (int y = 0; y < width; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    if (alpha > 0)
                    {
                        ...
                        // a = -1
                        // t = 192
                        var hue = (byte) Mod((int) (-factor*radAngle + 192), 256);
