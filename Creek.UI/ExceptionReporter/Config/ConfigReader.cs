        ...
        private void ReadUserInterfaceSettings()
        {
            ...
            if (!_info.ShowFullDetail)
                _info.ShowLessMoreDetailButton = true;
                    // prevent seeing only the simplified view, position of this line is important

            _info.ShowLessMoreDetailButton =
                GetUserInterfaceSetting("ShowLessMoreDetailButton").GetBool(_info.ShowLessMoreDetailButton);
            ...
            if (float.TryParse(fontSizeAsString, NumberStyles.Float, CultureInfo.CurrentCulture.NumberFormat,
                               out fontSize))
            {
