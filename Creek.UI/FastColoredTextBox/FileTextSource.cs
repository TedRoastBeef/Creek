...
namespace Creek.UI.FastColoredTextBox
{
    ...
    public class FileTextSource : TextSource
    {
        ...
        public void OpenFile(string fileName, Encoding enc)
        {
            ...
            int linesCount = Math.Min(lines.Count, CurrentTB.ClientRectangle.Height/CurrentTB.CharHeight);
        ...
        public override void SaveToFile(string fileName, Encoding enc)
        {
            ...
            string tempFileName = Path.Combine(dir, Path.GetFileNameWithoutExtension(fileName) + ".tmp");
            ...
            using (var sw = new StreamWriter(tempFs, enc))
            {
                ...
                for (int i = 0; i < Count; i++)
                {
                    ...
                    string sourceLine = ReadLine(sr, i); //read line from source file
                    string line;
        ...
        {
            string line;
            int filePos = sourceFileLinePositions[i];
            if (filePos < 0)
                return "";
