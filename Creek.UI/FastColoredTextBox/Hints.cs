...
namespace Creek.UI.FastColoredTextBox
{
    ...
    public class Hints : ICollection<Hint>, IDisposable
    {
        ...
        #region ICollection<Hint> Members
        ...
        public void Clear()
        {
            ...
            if (tb.Controls.Count != 0)
            {
                ...
                foreach (Control item in toDelete)
                    tb.Controls.Remove(item);

                for (int i = 0; i < tb.LineInfos.Count; i++)
                {
                    LineInfo li = tb.LineInfos[i];
                    li.bottomPadding = 0;
                    tb.LineInfos[i] = li;
                }
                tb.NeedRecalc();
        ...
        public void Add(Hint hint)
        {
            ...
            {
                LineInfo li = tb.LineInfos[hint.Range.Start.iLine];
        ...
        private void LayoutHint(Hint hint)
        {
            ...
            {
                Point p1 = tb.PlaceToPoint(hint.Range.Start);
                Point p2 = tb.PlaceToPoint(hint.Range.End);
                int cx = (p1.X + p2.X)/2;
