...
using Creek.UI.HTMLRenderer.Entities;
using Creek.UI.HTMLRenderer.Handlers;
using Creek.UI.HTMLRenderer.Parse;
...
namespace Creek.UI.HTMLRenderer.Dom
{
    ...
    internal class CssBox : CssBoxProperties, IDisposable
    {
        ...
        public virtual void Dispose()
        {
            ...
            foreach (CssBox childBox in Boxes)
            {
                childBox.Dispose();
            }
        }
        ...
        protected virtual void PerformLayoutImp(IGraphics g)
        {
            ...
            float actualWidth = Math.Max(GetMinimumWidth() + GetWidthMarginDeep(this),
                                         Size.Width < 90999 ? ActualRight : 0);
            HtmlContainer.ActualSize = CommonUtils.Max(HtmlContainer.ActualSize,
                                                       new SizeF(actualWidth,
                                                                 ActualBottom - HtmlContainer.Root.Location.Y));
        }
        ...
        internal virtual void MeasureWordsSize(IGraphics g)
        {
            if (!_wordsSizeMeasured)
            {
                ...
                if (Words.Count > 0)
                {
                    foreach (CssRect boxWord in Words)
                    {
                        boxWord.Width = boxWord.Text != "\n"
                                            ? FontsUtils.MeasureStringWidth(g, boxWord.Text, ActualFont)
                                            : 0;
        ...
        internal float GetMinimumWidth()
        {
            ...
            {
                CssBox box = maxWidthWord.OwnerBox;
                while (box != null)
                {
        ...
        /// <returns></returns>
        private static void GetMinMaxSumWords(CssBox box, ref float min, ref float maxSum, ref float paddingSum,
                                              ref float marginSum)
        {
            float? oldSum = null;
        ...
        private float MarginBottomCollapse()
        {
            ...
            {
                float lastChildBottomMargin = _boxes[_boxes.Count - 1].ActualMarginBottom;
                margin = Height == "auto" ? Math.Max(ActualMarginBottom, lastChildBottomMargin) : lastChildBottomMargin;
            }
            return Math.Max(ActualBottom,
                            _boxes[_boxes.Count - 1].ActualBottom + margin + ActualPaddingBottom +
        ...
        protected void PaintBackground(IGraphics g, RectangleF rect, bool isFirst, bool isLast)
        {
            if (rect.Width > 0 && rect.Height > 0)
            {
                ...
                {
                    brush = new LinearGradientBrush(rect, ActualBackgroundColor, ActualBackgroundGradient,
                ...
                if (brush != null)
                {
                    ...
                    if (IsRounded)
                    {
                        roundrect = RenderUtils.GetRoundRect(rect, ActualCornerNW, ActualCornerNE, ActualCornerSE,
                                                             ActualCornerSW);
                    }
                    ...
                    {
                        g.FillRectangle(brush, (float) Math.Ceiling(rect.X), (float) Math.Ceiling(rect.Y), rect.Width,
                                        rect.Height);
                    }
        ...
        private void PaintWords(IGraphics g, PointF offset)
        {
            foreach (CssRect word in Words)
            {
                ...
                if (word.Selected)
                {
                    ...
                    CssLineBox wordLine = DomUtils.GetCssLineBoxByWord(word);
                    float left = word.SelectedStartOffset > -1
                                     ? word.SelectedStartOffset
                                     : (wordLine.Words[0] != word && word.HasSpaceBefore ? -ActualWordSpacing : 0);
                    bool padWordRight = word.HasSpaceAfter && !wordLine.IsLastSelectedWord(word);
                    float width = word.SelectedEndOffset > -1
                                      ? word.SelectedEndOffset
                                      : word.Width + (padWordRight ? ActualWordSpacing : 0);
                    var rect = new RectangleF(word.Left + offset.X + left, word.Top + offset.Y, width - left,
                                              wordLine.LineHeight);

                    g.FillRectangle(GetSelectionBackBrush(false), rect.X, rect.Y, rect.Width, rect.Height);

                    if (HtmlContainer.SelectionForeColor != System.Drawing.Color.Empty &&
                        (word.SelectedStartOffset > 0 || word.SelectedEndIndexOffset > -1))
                    {
                        RectangleF orgClip = g.GetClip();
                        ...
                        g.DrawString(word.Text, ActualFont, GetSelectionForeBrush(), wordPoint,
                                     new SizeF(word.Width, word.Height));
                        g.SetClip(orgClip);
                    }
                    else
                    {
        ...
        protected void PaintDecoration(IGraphics g, RectangleF rectangle, bool isFirst, bool isLast)
        {
            ...
            {
                int h = g.MeasureString(" ", ActualFont).Height;
            ...
            Pen pen = RenderUtils.GetPen(ActualColor);
        ...
        {
            return HtmlContainer.SelectionForeColor != System.Drawing.Color.Empty
        ...
        {
            Color backColor = HtmlContainer.SelectionBackColor;
            if (backColor != System.Drawing.Color.Empty)
            {
                if (forceAlpha && backColor.A > 180)
                    return
                        RenderUtils.GetSolidBrush(System.Drawing.Color.FromArgb(180, backColor.R, backColor.G,
                                                                                backColor.B));
                else
        ...
        {
            string tag = HtmlTag != null ? string.Format("<{0}>", HtmlTag.Name) : "anon";
        ...
        public static CssBox CreateBlock(CssBox parent, HtmlTag tag = null, CssBox before = null)
        {
            ...
            CssBox newBox = CreateBox(parent, tag, before);
            ...
            try
            {
                if (Display != CssConstants.None && Visibility == CssConstants.Visible)
                {
                    ...
                    if (!visible)
                    {
                        ...
                        RectangleF rect = ContainingBlock.ClientRectangle;
                        rect.X -= 2;
                        rect.Width += 2;
        ...
        public void ParseToWords()
        {
            ...
            if (WhiteSpace == CssConstants.Pre)
            {
                ...
                while (startIdx < _text.Length)
                {
                    ...
                    if (startIdx < _text.Length)
                    {
                        if (endIdx > startIdx)
                        {
                            string word = HtmlUtils.DecodeHtml(_text.Substring(startIdx, endIdx - startIdx));
            ...
            else
            {
                ...
                while (startIdx < _text.Length)
                {
                    ...
                    int idxInc = 0;
                    if (endIdx < _text.Length &&
                        (_text[endIdx] == '-' || WordBreak == CssConstants.BreakAll ||
                    ...
                    if (startIdx < _text.Length)
                    {
                        bool hasSpaceBefore = startIdx > 0 && _boxWords.Count == 0 &&
                                              char.IsWhiteSpace(_text[startIdx - 1]);
                        bool hasSpaceAfter = endIdx < _text.Length && char.IsWhiteSpace(_text[endIdx]);
                        string word = HtmlUtils.DecodeHtml(_text.Substring(startIdx, endIdx - startIdx));
