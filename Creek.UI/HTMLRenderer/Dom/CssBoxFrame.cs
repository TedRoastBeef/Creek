...
namespace Creek.UI.HTMLRenderer.Dom
{
    ...
    internal sealed class CssBoxFrame : CssBox
    {
        ...
        private void OnDownloadYoutubeApiCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            try
            {
                int idx = e.Result.IndexOf("\"media$title\"");
                if (idx > -1)
                {
                    ...
                    if (idx > -1)
                    {
                        ...
                        if (idx > -1)
                        {
                            int endIdx = e.Result.IndexOf('"', idx + 1);
                ...
                if (idx > -1)
                {
                    int iidx = e.Result.IndexOf("sddefault", idx);
                    if (iidx > -1)
                    {
                    ...
                    if (iidx > -1)
                    {
                        int endIdx = e.Result.IndexOf('"', iidx);
                        if (endIdx > -1)
                        {
                ...
                if (idx > -1)
                {
                    ...
                    if (idx > -1)
                    {
                        int endIdx = e.Result.IndexOf('"', idx);
                        if (endIdx > -1)
                        {
        ...
        private void OnDownloadVimeoApiCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            try
            {
                int idx = e.Result.IndexOf("\"title\"");
                if (idx > -1)
                {
        ...
        {
            RectangleF rects = CommonUtils.GetFirstValueOrDefault(Rectangles);
            ...
            CssRect word = Words[0];
            RectangleF tmpRect = word.Rectangle;
            ...
            Rectangle rect = Rectangle.Round(tmpRect);
        ...
        private void DrawTitle(IGraphics g, Rectangle rect)
        {
            ...
            {
                Font font = FontsUtils.GetCachedFont("Arial", 9f, System.Drawing.FontStyle.Regular);
                g.FillRectangle(RenderUtils.GetSolidBrush(System.Drawing.Color.FromArgb(160, 0, 0, 0)), rect.Left,
                                rect.Top, rect.Width, FontsUtils.GetFontHeight(font) + 7);
        ...
        private void DrawPlay(IGraphics g, Rectangle rect)
        {
            if (_isVideo && _imageWord.Width > 70 && _imageWord.Height > 50)
            {
                ...
                int top = rect.Top + (rect.Height - size.Height)/2;
                g.FillRectangle(RenderUtils.GetSolidBrush(System.Drawing.Color.FromArgb(160, 0, 0, 0)), left, top,
                                size.Width, size.Height);

                using (var path = new GraphicsPath())
                {
                    path.AddLine(left + size.Width/3f + 1, top + 3*size.Height/4f, left + size.Width/3f + 1,
                                 top + size.Height/4f);
                    path.AddLine(left + size.Width/3f + 1, top + size.Height/4f, left + 2*size.Width/3f + 1,
                                 top + size.Height/2f);
                    path.CloseFigure();
