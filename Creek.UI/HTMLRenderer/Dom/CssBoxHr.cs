...
namespace Creek.UI.HTMLRenderer.Dom
{
    ...
    internal sealed class CssBoxHr : CssBox
    {
        ...
        protected override void PaintImp(IGraphics g)
        {
            ...
            {
                g.FillRectangle(RenderUtils.GetSolidBrush(ActualBackgroundColor), rect.X, rect.Y, rect.Width,
                                rect.Height);
            }

            Brush b1 = RenderUtils.GetSolidBrush(ActualBorderTopColor);
            BordersDrawHandler.DrawBorder(Border.Top, g, this, b1, rect);

            if (rect.Height > 1)
            {
                Brush b2 = RenderUtils.GetSolidBrush(ActualBorderLeftColor);
                BordersDrawHandler.DrawBorder(Border.Left, g, this, b2, rect);

                Brush b3 = RenderUtils.GetSolidBrush(ActualBorderRightColor);
                BordersDrawHandler.DrawBorder(Border.Right, g, this, b3, rect);

                Brush b4 = RenderUtils.GetSolidBrush(ActualBorderBottomColor);
