...
namespace Creek.UI.HTMLRenderer.Dom
{
    ...
    internal static class CssLayoutEngine
    {
        ...
        public static void MeasureImageSize(CssRectImage imageWord)
        {
            ...
            {
                imageWord.Width = imageWord.ImageRectangle == Rectangle.Empty
            ...
            {
                imageWord.Height = imageWord.ImageRectangle == Rectangle.Empty
        ...
        private static void FlowBox(IGraphics g, CssBox blockbox, CssBox box, float limitRight, float linespacing,
                                    float startx, ref CssLineBox line, ref float curx, ref float cury,
                                    ref float maxRight, ref float maxbottom)
        {
            float startX = curx;
            float startY = cury;
            box.FirstHostingLineBox = line;
            float localCurx = curx;
            float localMaxRight = maxRight;
            float localmaxbottom = maxbottom;

            foreach (CssBox b in box.Boxes)
            {
                float leftspacing = b.Position != CssConstants.Absolute
                                        ? b.ActualMarginLeft + b.ActualBorderLeftWidth + b.ActualPaddingLeft
                                        : 0;
                float rightspacing = b.Position != CssConstants.Absolute
                ...
                if (b.Words.Count > 0)
                {
                    ...
                    foreach (CssRect word in b.Words)
                    {
                        ...
                        if ((b.WhiteSpace != CssConstants.NoWrap && b.WhiteSpace != CssConstants.Pre &&
                             curx + word.Width + rightspacing > limitRight) || word.IsLineBreak || wrapNoWrapBox)
                        {
                            wrapNoWrapBox = false;
                            curx = startx;
        ...
        private static void ApplyVerticalAlignment(IGraphics g, CssLineBox lineBox)
        {
            ...
            foreach (CssBox box in lineBox.Rectangles.Keys)
            {
        ...
        private static void ApplyCenterAlignment(IGraphics g, CssLineBox line)
        {
            ...
            float right = line.OwnerBox.ActualRight - line.OwnerBox.ActualPaddingRight -
                          line.OwnerBox.ActualBorderRightWidth;
            float diff = right - lastWord.Right - lastWord.OwnerBox.ActualBorderRightWidth -
