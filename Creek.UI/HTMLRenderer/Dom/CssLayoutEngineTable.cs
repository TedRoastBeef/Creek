...
namespace Creek.UI.HTMLRenderer.Dom
{
    ...
    internal sealed class CssLayoutEngineTable
    {
        ...
        private void Layout(IGraphics g)
        {
            ...
            float availCellSpace = CalculateCountAndWidth();
                        ...
                        if (i < 20 || float.IsNaN(_columnWidths[i])) // limit column width check
                        {
                            if (i < row.Boxes.Count && row.Boxes[i].Display == CssConstants.TableCell)
                            {
                                ...
                                if (len > 0) //If some width specified
                                {
                                    ...
                                    {
                                        _columnWidths[j] = float.IsNaN(_columnWidths[j])
                                                               ? len
        ...
        private void DetermineMissingColumnWidths(float availCellSpace)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < _columnWidths.Length; i++)
                {
                    ...
                    {
                        float temp = _columnWidths[i];
                        _columnWidths[i] =
                            Math.Min(
                                _columnWidths[i] +
                                (availCellSpace - occupedSpace)/Convert.ToSingle(_columnWidths.Length - i),
                                maxFullWidths[i]);
        ...
        private void EnforceMaximumSize()
        {
            ...
            float maxWidth = GetMaxTableWidth();
            if (maxWidth < 90999)
            {
                ...
                if (maxWidth < widthSum)
                {
                    ...
                    if (maxWidth < widthSum)
                    {
                            ...
                            // limit iteration so bug won't create infinite loop
                        {
                            ...
                            float decrease = secLargeWidth > 0
        ...
        private void LayoutCells(IGraphics g)
        {
            ...
            for (int i = 0; i < _allRows.Count; i++)
            {
                ...
                for (int j = 0; j < row.Boxes.Count; j++)
                {
                    ...
                    int rowspan = GetRowSpan(cell);
                    int columnIndex = GetCellRealColumnIndex(row, cell);
            ...
            _tableBox.ActualBottom = Math.Max(maxBottom, starty) + GetVerticalSpacing() +
                                     _tableBox.ActualBorderBottomWidth;
        }
            ...
            if (_columnMinWidths == null)
            {
                ...
                foreach (CssBox row in _allRows)
                {
                    foreach (CssBox cell in row.Boxes)
                    {
                        ...
                        float spannedwidth = GetSpannedMinWidth(row, cell, col, colspan) +
