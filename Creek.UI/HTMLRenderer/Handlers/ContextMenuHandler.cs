    ...
    internal sealed class ContextMenuHandler : IDisposable
    {
        ...
        private void OnSaveImageClick(object sender, EventArgs eventArgs)
        {
            try
            {
                ...
                {
                    string imageSrc = _currentRect.OwnerBox.GetAttribute("src");
            ...
            try
            {
                ...
                if (rect != null)
                {
                    ...
                    if (link != null)
                    {
                        ...
                        ToolStripItem openLink = _contextMenu.Items.Add(isVideo ? _openVideo : _openLink, null,
                                                                        OnOpenLinkClick);
                        if (_htmlContainer.IsSelectionEnabled)
                        {
                            ToolStripItem copyLink = _contextMenu.Items.Add(isVideo ? _copyVideoUrl : _copyLink, null,
                                                                            OnCopyLinkClick);
                    ...
                    {
                        ToolStripItem saveImage = _contextMenu.Items.Add(_saveImage, null, OnSaveImageClick);
                        if (_htmlContainer.IsSelectionEnabled)
                        {
                            ToolStripItem copyImageUrl = _contextMenu.Items.Add(_copyImageLink, null,
                                                                                OnCopyImageLinkClick);
                            ToolStripItem copyImage = _contextMenu.Items.Add(_copyImage, null, OnCopyImageClick);
                    ...
                    if (_htmlContainer.IsSelectionEnabled)
                    {
                        ToolStripItem copy = _contextMenu.Items.Add(_copy, null, OnCopyClick);
