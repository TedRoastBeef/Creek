...
namespace Creek.UI.HTMLRenderer.Handlers
{
    /// <summary>
...
// ReSharper restore RedundantNameQualifier
        ...
        {
            string[] s = src.Substring(src.IndexOf(':') + 1).Split(new[] {','}, 2);
            if (s.Length == 2)
            {
                int imagePartsCount = 0, base64PartsCount = 0;
                foreach (string part in s[0].Split(new[] {';'}))
                {
                    string pPart = part.Trim();
        ...
        {
            FileInfo filePath = CommonUtils.GetLocalfileName(source);
            ...
            else
            {
                if (_htmlContainer.AvoidAsyncImagesLoading)
                    ...
                    ThreadPool.QueueUserWorkItem(DownloadImageFromUrlAsync,
                                                 new KeyValuePair<Uri, FileInfo>(source, filePath));
            }
        }
        ...
        private void DownloadImageFromUrl(Uri source, FileInfo filePath)
        {
            try
            {
                using (WebClient client = _client = new WebClient())
                {
        ...
        {
            Uri uri = ((KeyValuePair<Uri, FileInfo>) data).Key;
            FileInfo filePath = ((KeyValuePair<Uri, FileInfo>) data).Value;
        ...
        private void OnDownloadImageCompleted(bool cancelled, Exception error, FileInfo filePath, WebClient client)
        {
            if (!cancelled && !_disposed)
            {
                ...
                {
                    filePath.Refresh();
                    string contentType = CommonUtils.GetResponseContentType(client);
                    ...
                    {
                        _htmlContainer.ReportError(HtmlRenderErrorType.Image,
                                                   "Failed to load image, not image content type: " + contentType);
                        ImageLoadComplete();
                        filePath.Delete();
                    }
                }
                ...
                {
                    _htmlContainer.ReportError(HtmlRenderErrorType.Image,
                                               "Failed to load image from URL: " + client.BaseAddress, error);
                    ImageLoadComplete();
                }
            }
        }
