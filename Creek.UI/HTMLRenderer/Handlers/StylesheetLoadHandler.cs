...
namespace Creek.UI.HTMLRenderer.Handlers
{
    ...
    internal static class StylesheetLoadHandler
    {
        ...
        public static void LoadStylesheet(HtmlContainer htmlContainer, string src, Dictionary<string, string> attributes,
        ...
        private static string LoadStylesheetFromUri(HtmlContainer htmlContainer, Uri uri)
        {
            ...
            {
                string stylesheet = client.DownloadString(uri);
                try
                {
        ...
        private static string CorrectRelativeUrls(string stylesheet, Uri baseUri)
        {
            ...
            while (idx != -1 && idx < stylesheet.Length)
            {
                ...
                if (idx > 0)
                {
                    ...
                    if (endIdx > 0)
                    {
                        string urlStr = stylesheet.Substring(idx + 4, endIdx - idx - 4);
                        Uri url;
