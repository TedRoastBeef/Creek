...
namespace Creek.UI.HTMLRenderer.Parse
{
    ...
    internal static class CssValueParser
    {
        ...
        /// <returns>the parsed length value with adjustments</returns>
        public static float ParseLength(string length, float hundredPercent, float emFactor, string defaultUnit,
        ...
        {
            string unit = length.Length >= 3 ? length.Substring(length.Length - 2, 2) : string.Empty;
            switch (unit)
            {
                case CssConstants.Em:
                case CssConstants.Ex:
