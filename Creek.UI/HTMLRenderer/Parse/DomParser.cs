...
namespace Creek.UI.HTMLRenderer.Parse
{
    ...
    internal static class DomParser
    {
        ...
        {
            string classes = box.HtmlTag.TryGetAttribute("class");
            ...
            while (startIdx < classes.Length)
            {
                ...
                {
                    int endIdx = classes.IndexOf(' ', startIdx);

                    if (endIdx < 0)
                        endIdx = classes.Length;

                    string cls = "." + classes.Substring(startIdx, endIdx - startIdx);
        ...
        {
            IEnumerable<CssBlock> blocks = cssData.GetCssBlock(className);
            foreach (CssBlock block in blocks)
            {
        ...
        private static bool IsBlockAssignableToBoxWithSelector(CssBox box, CssBlock block)
        {
            foreach (CssBlockSelectorItem selector in block.Selectors)
            {
                ...
                while (!matched)
                {
                    ...
                    {
                        string className = box.HtmlTag.TryGetAttribute("class");
                ...
                foreach (string att in tag.Attributes.Keys)
                {
                    ...
                    switch (att)
                    {
                        ...
                        case HtmlConstants.Bordercolor:
                            box.BorderLeftColor =
                                box.BorderTopColor = box.BorderRightColor = box.BorderBottomColor = value.ToLower();
                            break;
            ...
            SetForAllCells(table, cell =>
                                      {
                                          ...
                                          cell.BorderLeftWidth =
                                              cell.BorderTopWidth =
                                              cell.BorderRightWidth = cell.BorderBottomWidth = border;
                                      });
        }
        ...
        {
            foreach (CssBox l1 in table.Boxes)
            {
                foreach (CssBox l2 in l1.Boxes)
                {
        ...
        private static void CorrectTextBoxes(CssBox box)
        {
            for (int i = box.Boxes.Count - 1; i >= 0; i--)
            {
                ...
                {
                    // is the box has text
                    bool keepBox = !childBox.Text.IsEmptyOrWhitespace();
                    ...
                    keepBox = keepBox ||
                              (i > 0 && i < box.Boxes.Count - 1 && box.Boxes[i - 1].IsInline &&
        ...
        private static void CorrectImgBoxes(CssBox box)
        {
            for (int i = box.Boxes.Count - 1; i >= 0; i--)
            {
                ...
                {
                    CssBox block = CssBox.CreateBlock(childBox.ParentBox, null, childBox);
        ...
        private static void CorrectBlockInsideInlineImp(CssBox box)
        {
            if (box.Boxes.Count > 1 || box.Boxes[0].Boxes.Count > 1)
            {
                ...
                {
                    CssBox rightBox = CssBox.CreateBox(box, null, box.Boxes[2]);
        ...
        {
            CssBox leftbox = CssBox.CreateBox(leftBlock, badBox.HtmlTag);
            ...
            CssBox splitBox = badBox.Boxes[0];
        ...
        private static void CorrectInlineBoxesParent(CssBox box)
        {
            if (ContainsVariantBoxes(box))
            {
                for (int i = 0; i < box.Boxes.Count; i++)
                {
                    ...
                    {
                        CssBox newbox = CssBox.CreateBlock(box, null, box.Boxes[i++]);
        ...
        private static bool ContainsVariantBoxes(CssBox box)
        {
            ...
            {
                bool isBlock = !box.Boxes[i].IsInline;
