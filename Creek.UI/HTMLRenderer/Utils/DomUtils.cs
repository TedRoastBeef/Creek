...
namespace Creek.UI.HTMLRenderer.Utils
{
    ...
    internal sealed class DomUtils
    {
        ...
        public static bool IsBoxHasWhitespace(CssBox box)
        {
            ...
            {
                CssBox sib = GetPreviousContainingBlockSibling(box);
        ...
        public static CssBox GetNextSibling(CssBox b)
        {
            ...
            if (b.ParentBox != null)
            {
                int index = b.ParentBox.Boxes.IndexOf(b) + 1;
                while (index <= b.ParentBox.Boxes.Count - 1)
                {
                    CssBox pSib = b.ParentBox.Boxes[index];
        ...
        public static CssBox GetLinkBox(CssBox box, Point location)
        {
            if (box != null)
            {
                ...
                if (box.ClientRectangle.IsEmpty || box.ClientRectangle.Contains(location))
                {
                    foreach (CssBox childBox in box.Boxes)
                    {
                        CssBox foundBox = GetLinkBox(childBox, location);
                        if (foundBox != null)
                            return foundBox;
                    }
                }
        ...
        public static CssBox GetBoxById(CssBox box, string id)
        {
            if (box != null && !string.IsNullOrEmpty(id))
            {
                ...
                foreach (CssBox childBox in box.Boxes)
                {
                    CssBox foundBox = GetBoxById(childBox, id);
                    if (foundBox != null)
                        return foundBox;
                }
            }
        ...
        public static CssRect GetCssBoxWord(CssBox box, Point location)
        {
            if (box != null && box.Visibility == CssConstants.Visible)
            {
                if (box.LineBoxes.Count > 0)
                {
                    foreach (CssLineBox lineBox in box.LineBoxes)
                    {
                        CssRect wordBox = GetCssBoxWord(lineBox, location);
                        if (wordBox != null)
                            return wordBox;
                    }
                }
                ...
                if (box.ClientRectangle.IsEmpty || box.ClientRectangle.Contains(location))
                {
                    foreach (CssBox childBox in box.Boxes)
                    {
                        CssRect foundWord = GetCssBoxWord(childBox, location);
                        if (foundWord != null)
                        {
                            return foundWord;
                        }
        ...
        public static CssRect GetCssBoxWord(CssLineBox lineBox, Point location)
        {
            ...
            {
                foreach (CssRect word in rects.Key.Words)
                {
        ...
        public static CssLineBox GetCssLineBoxByWord(CssRect word)
        {
            ...
            foreach (CssLineBox lineBox in box.LineBoxes)
            {
                foreach (CssRect lineWord in lineBox.Words)
                {
                    if (lineWord == word)
                    {
                        return lineBox;
        ...
        public static string GetSelectedPlainText(CssBox root)
        {
            ...
            int lastWordIndex = GetSelectedPlainText(sb, root);
        ...
        {
            int lastWordIndex = 0;
            foreach (CssRect boxWord in box.Words)
            {
            ...
            if (box.Visibility != CssConstants.Hidden && box.Display != CssConstants.None)
            {
                foreach (CssBox childBox in box.Boxes)
                {
                    int innerLastWordIdx = GetSelectedPlainText(sb, childBox);
        ...
        private static bool CollectSelectedHtmlTags(CssBox box, Dictionary<HtmlTag, bool> selectedTags,
                                                    Dictionary<HtmlTag, bool> maybeTags)
        {
            ...
            foreach (CssBox childBox in box.Boxes)
            {
                bool childInSelection = CollectSelectedHtmlTags(childBox, selectedTags, maybeTags);
                if (childInSelection)
                {
                    if (box.HtmlTag != null)
                    {
        ...
        private static void WriteStylesheet(StringBuilder sb, CssData cssData, int indent)
        {
            ...
            foreach (var cssBlocks in cssData.MediaBlocks["all"])
            {
                ...
                sb.Append(" { ");
                foreach (CssBlock cssBlock in cssBlocks.Value)
                {
        ...
        private static string GetSelectedWord(CssRect rect, bool selectedText)
        {
            ...
            {
                bool whitespaceBefore = rect.OwnerBox.Words[0] == rect
