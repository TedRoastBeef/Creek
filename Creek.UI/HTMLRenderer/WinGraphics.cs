    ...
    internal sealed class WinGraphics : IGraphics
    {
        ...
        public Size MeasureString(string str, Font font)
        {
            if (_useGdiPlusTextRendering)
            {
                ...
                SizeF size = _g.MeasureCharacterRanges(str, font, RectangleF.Empty, _stringFormat)[0].GetBounds(_g).Size;
        ...
        public Size MeasureString(string str, Font font, float maxWidth, out int charFit, out int charFitWidth)
        {
            ...
            else
            {
                ...
                var size = new Size();
                Win32Utils.GetTextExtentExPoint(_hdc, str, str.Length, (int) Math.Round(maxWidth), _charFit,
                                                _charFitWidth, ref size);
                charFit = _charFit[0];
        ...
        public void DrawString(String str, Font font, Color color, PointF point, SizeF size)
        {
            ...
            {
                ReleaseHdc();
                _g.DrawString(str, font, RenderUtils.GetSolidBrush(color),
                              point.X - FontsUtils.GetFontLeftPadding(font)*.8f, point.Y);
            }
            else
            {
                if (color.A == 255)
                ...
                {
                    InitHdc();
                    DrawTransparentText(_hdc, str, font, new Point((int) Math.Round(point.X), (int) Math.Round(point.Y)),
                                        Size.Round(size), color);
                }
            }
        }
        ...
        {
            IntPtr dib;
            IntPtr memoryHdc = Win32Utils.CreateMemoryHdc(hdc, size.Width, size.Height, out dib);
