    ...
    public class FATabStrip : BaseStyledPanel, ISupportInitialize, IDisposable
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (RightToLeft == RightToLeft.No)
            {
                ...
                {
                    var end = new Point((int) SelectedItem.StripRect.Left - 9, DEF_HEADER_HEIGHT);
            ...
            else
            {
                ...
                {
                    var end = new Point((int) SelectedItem.StripRect.Left, DEF_HEADER_HEIGHT);
                    e.Graphics.DrawLine(SystemPens.ControlDark, new Point(0, DEF_HEADER_HEIGHT), end);
        ...
        private void OnCalcTabPage(Graphics g, FATabStripItem currentItem)
        {
            ...
            if (RightToLeft == RightToLeft.No)
            {
                var buttonRect = new RectangleF(DEF_START_POS, 3, textSize.Width, 17);
                currentItem.StripRect = buttonRect;
                DEF_START_POS += (int) textSize.Width;
            }
            else
            {
                var buttonRect = new RectangleF(DEF_START_POS - textSize.Width + 1, 3, textSize.Width - 1, 17);
                currentItem.StripRect = buttonRect;
                DEF_START_POS -= (int) textSize.Width;
            }
        }
